# fly-pgsql 设计文档：基于 Neon 开源栈的 Serverless Postgres
...
fly-pgsql 是 InsForge BaaS 的多租户数据库底座：为每个用户项目提供一个独立的 serverless Postgres，要求三个核心能力：
...
- 计费计量（只暴露用量数据接口，计费逻辑归 InsForge 平台）。

## 2. 架构总览

```
                         ┌──────────────────────────────────┐
  InsForge 平台 ──────────►  fly-pgsql 控制面 (Go, 自研)       │
...
### 3.2 fly-pgsql 控制面（Go，自研——本项目核心交付物）

四块职责，单二进制部署：

**① InsForge 北向 API（REST）**
...
- `GET /endpoints/{endpoint}/jwks`：第一阶段返回空列表；后期对接 InsForge auth 实现 JWT 直连。
...
| **M3 分支与北向 API** | branching、北向 REST、集成测试套件 | 第 4 节本地闭环验收全过；InsForge 可凭 API 完成全生命周期 |
